...
> → 2026-08-06 노트북에서 **잠정 반영됨**: 정적 TF `x=0.30/z=0.25`(사용자 실측),
> `ignore_array: "-78,-70,72,79"`(08-05 기둥 재측정 ±3° 여유). bag 기반 정밀 산정은
> 생략(마감) — 현장 검증·부호 반전 절차는 [PHASE3_QUICKSTART.md](PHASE3_QUICKSTART.md) §1.
> **선반 카트 조립 확정(08-06)**: 라이다·카메라 위치 고정. 라이다는 임시 장착에서
> **z축으로만 이동**(수평 불변 → x/y/yaw·기둥 각도 유지, 새 z 실측 대기).
> **카메라 화각 내 기둥 없음**(사용자 확인 — AI 탐지 무간섭).
...
> **✅ 2026-08-05 Stage 0 통과.** 상세 실행 계획은 `~/.claude/plans/readme-step-validated-breeze.md`,
> 실측 기록은 `tests/TEST_LOG.md` 2026-08-05 09:10 항목. 다음 단계는 Stage 1(AI 스택 단독 관측).
>
> **2026-08-04에 보고한 블로커는 실재하지 않았다** — 그 측정은 팀원들이 카트를 다른 장소로
> 옮긴 상태에서 이뤄진 것이었다(라이다 장착 위치는 불변). 실제 카트 위치 재측정 결과:
>
> | 항목 | 08-04(다른 장소) | **08-05(실제 위치)** |
> |---|---|---|
> | AI 조회창 ±29° 유효율 | 8.6% | **71.4%** |
> | 정면 −5°~+5° | 유효 빔 0개 | **전 빔 유효, 5.37 m** |
>
> 원시값 분류에서 `above_max`·`inf`·`NaN` **0건** — 산발적 `zero` 드롭아웃뿐이다.
>
> **카트 기둥은 `−75..−73°`(0.130 m)·`+75..+76°`(0.150 m)** 로 조회창에서 46° 이상 이격 →
> Phase 2 무영향. 단 `ignore_array`가 비어 있어 **매핑(Phase 3) 전에는 마스킹 필수**.
>
> **각도 정합도 실측 확정**: 사람을 카메라 정면 1.5 m에 세우면 `−7.8..+2.9°`(중심 −2.5°)·1.571 m,
> 카메라 오른쪽 1 m로 옮기면 `+20.9..+31.6°`(중심 +26.2°)·1.94 m.
> → `lidar_yaw_offset_deg=0.0` **맞음**, `lidar_mirrored=True` **맞음**(현재 런치값 유지).
> 드라이버가 `inverted: false`인데도 반전인 건 장착 방향 때문으로 보이나 원인 미확정 —
> **AI 파트 협의 불필요**.
