 ...
 *
 * === 2026-08-08 재측정 (바퀴 공중, 손으로 1회전씩 좌우 각 10회, 이상치 제거 평균) ===
 *   Left  약 68420 count/rev
 *   Right 약 67913 count/rev
 *   좌우 전체 평균 약 68167 count/rev
 *
 *   통합 평균이 2026-08-03의 68162.5와 사실상 같은 값으로 재현됐다. 즉 명목값과의
 *   약 -12% 차이는 1회성 측정 실수가 아니다.
 *
 *   ⚠️ 좌우 편차는 재현되지 않았다. 08-03은 Right가 약 0.16% 컸고, 08-08은 Left가
 *      약 0.75% 크다 — 크기도 부호도 달라졌다. 두 측정 모두 손 회전이므로 "정확히
 *      1회전"을 맞추는 오차가 값에 섞여 있다. **좌우 차이를 하드웨어 특성으로 확정할
 *      수 없다.** 좌우 개별 보정값을 도입할 근거는 아직 없다.
 *
 ...
 *
 *   ⚠️ 2026-08-08 현재 ROS 쪽과 스케일이 서로 다르다 (의도된 일시적 불일치):
 *     - 이 펌웨어(actual_rad_s, PI 오차 입력, Stall 판정): 명목 77520 유지
 *     - ROS Wheel Odometry(`counts_per_wheel_rev` 파라미터): 실측 68160 사용
 *     펌웨어를 지금 바꾸지 않는 이유는 이 상수가 속도 보고뿐 아니라 PI 제어 입력과
 *     Stall 판정까지 함께 바꾸기 때문이다. 스케일 통일은 원인 규명을 포함하는 별건의
 *     Encoder Scale Calibration 작업에서 한다.
 *     그때까지 STATUS의 LA/RA와 ROS odometry 속도를 같은 축에서 직접 비교하지 말 것. */
