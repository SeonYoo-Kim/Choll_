...
- 좌우 측정값은 서로 약 **0.16%** 차이였다

#### ✅ 2026-08-08 재측정 — 통합 평균 재현됨 (원인은 여전히 미확정)

바퀴를 공중에 띄운 상태에서 손으로 **1회전씩 좌우 각 10회** 측정하고 이상치를 제거해 평균했다.

| 대상 | 평균 count/wheel-rev | 08-03 대비 |
|------|----------------------|------------|
| Left | 약 68420 | +312 |
| Right | 약 67913 | -304 |
| **좌우 전체 평균** | **약 68167** | +4.5 |

- **통합 평균이 08-03의 68162.5와 사실상 동일하게 재현됐다.** 명목값과의 약 -12% 차이는
  1회성 측정 실수가 아니다. 앞선 검증 계획의 **판정 A**(기존 실측 재확인)에 해당한다.
- ⚠️ **좌우 편차는 재현되지 않았다.** 08-03은 Right가 약 0.16% 컸고, 08-08은 Left가 약 0.75%
  크다 — 크기도 부호도 달라졌다. 두 측정 모두 손 회전이므로 "정확히 1회전"을 맞추는 오차가
  값에 포함돼 있다. **좌우 차이를 하드웨어 특성으로 확정하지 않는다.** 좌우 개별 보정값을
  도입할 근거는 아직 없으며, 필요하다면 손 회전이 아닌 방식으로 다시 측정해야 한다.
- **ROS Wheel Odometry는 좌우 공통 기준값 `68160` count/rev를 쓰기로 했다**(2026-08-08 결정).
  아래 "펌웨어와 ROS의 스케일 불일치" 절 참고.
...
**실측값을 펌웨어의 보정 상수로 강제 적용하지 않았다.** `motor_config.h`의
`MOTOR_ENCODER_COUNTS_PER_WHEEL_REV`는 기존 파생식(`CPR x GEAR x QUADRATURE`)을 그대로
유지한다 — 원인을 모른 채 숫자만 맞추면 다른 조건에서 다시 틀어진다.

#### ⚠️ 펌웨어와 ROS의 스케일 불일치 (2026-08-08 현재, 의도된 일시적 상태)

같은 엔코더에 대해 **두 계층이 서로 다른 count/rev를 쓴다.**

| 계층 | 상수 | 값 | 영향 범위 |
|---|---|---|---|
| STM32 펌웨어 | `MOTOR_ENCODER_COUNTS_PER_WHEEL_REV` | **77520** (명목) | `actual_rad_s` → STATUS의 LA/RA, PI 오차 입력, Stall 판정 |
| ROS 브리지 | `counts_per_wheel_rev` (파라미터) | **68160** (실측) | Wheel Odometry (구현 예정) |

펌웨어를 지금 바꾸지 않는 이유는, 이 상수가 속도 **보고**만이 아니라 **PI 제어 입력과 Stall
판정까지 함께** 바꾸기 때문이다. 스케일 통일은 원인 규명을 포함한 별건의 **Encoder Scale
Calibration** 작업에서 한다.

그때까지 지켜야 할 것:

- `/stm/wheel_actual_rad_s`(77520 기준, 실제보다 약 12% 작음)와 ROS odometry가 산출한 속도
  (68160 기준)를 **같은 축에서 직접 비교하지 않는다.** 약 12% 차이는 결함이 아니라 이 불일치다.
- 두 값을 함께 쓰는 로직(예: odometry와 actual_rad_s의 교차 검증)은 **이 불일치가 해소되기
  전까지 만들지 않는다.**
