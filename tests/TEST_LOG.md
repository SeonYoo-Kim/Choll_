...
## 2026-08-09 — ✅ BE: v2 지도(재매핑본) 아핀 재계산 (Claude)

- **배경**: EM이 시연장 배치(서가 블록 2개) 완료 후 **재매핑 + GIMP 정리**한 library_v2
  (289×207, res 0.05, origin [-8.34, -6.94]) 도착. v1과 픽셀 정합 시도 결과 **다른 SLAM
  세션**으로 판명 (내부 구조물 상이: v1 박스 1개 / v2 박스 2개) — v1 기준 아핀은 v2 좌표에서
  200~650px·약 59° 어긋나 전면 무효
- **v2 단독 분석**: 방 기울기 -8°(벽 투영 첨도), 안쪽 벽 프레임 u[123.5,233.5]·v[79.5,216.5]
  (5.5×6.85m — v1 실측 5.5×7.0m과 일치), 서가 박스 2개 세로 배치 = 평면도 90° 회전 방향.
  방향·좌우 확정 근거: 박스-벽 거리 비대칭(800블록 48px vs 000블록 37px)과
  **사서 테이블 노치**(좌하단, plan (195,29)로 투영 → 사서 테이블 영역 정확히 일치)
- **새 아핀**: A=[[-20.317, 144.565], [108.029, 15.183]], t=(917.881, 175.555)
  — 검증 잔차: 000/100 블록 28px(19cm), 800/200 블록 37px(25cm), 노치 영역 일치
- **반영**: `library-map-affine-v2.sql` 신규(로컬 DB 적용), `manual_position.py` AFFINE 상수 갱신
