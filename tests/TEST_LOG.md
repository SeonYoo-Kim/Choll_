...
## 2026-08-08 14:00 — 🔴 기구 실측 정정: L자 브라켓 풀림 발견 → `wheel_separation` 0.265 기각·0.38 복귀 / ⚠️ 바퀴 지름 실측이 스케일 보정과 모순 (사용자 실측 / Claude 반영·분석)
...
### ② ⚠️ 바퀴 지름 실측이 거리 스케일 보정과 모순된다 — 다음 실측 필요

사용자 육안 실측: **지름 약 100 mm**(하중으로 눌린 상태). 프로젝트 BOM도
**100 mm AGV 솔리드 고무휠**이다(130파이 오프로드휠은 반품·교체됨).
그런데 브리지·오도메트리는 `wheel_radius_m` 명목값으로 **0.065(=130 mm)** 를 쓰고 있다.
...
#### 다음 실측 (가정이 필요 없는 순서 — Jetson 세션)

1. **줄자로 바퀴 둘레 직접 측정** — 트레드에 줄자를 감아 원주를 잰다.
   지름 추정보다 정확하고, 하중 상태 축-바닥 높이도 함께 재면 눌림량이 나온다
2. **바퀴 1회전 굴리기** — 타이어에 표시하고 카트를 밀어 표시가 한 바퀴 돌아온 지점까지
   이동. **이동거리(=실제 원주)와 `/stm/encoder_total` 델타**를 동시에 기록하면
   `counts/rev`와 원주를 **동시에·독립적으로** 확정한다 (r·cpr 가정 불필요)
3. 그 결과로 `counts/m = Δ/실측거리`를 계산해 현재 설정 함의값 **184,804 count/m**와 비교
4. 값이 확정되면 명령 경로(`wheel_radius_m`)와 오도메트리 설정을 함께 정합
