"""YOLO .pt 하나를 TensorRT .engine으로 변환하는 Jetson용 스크립트.

반드시 Jetson 본체에서 실행 (엔진은 하드웨어·TensorRT 버전 종속).
기존 스크립트가 한 프로세스 안에서 9개 모델을 연속 export하다가
NvMapMemAlloc error 12 (메모리 파편화/부족)로 크래시하는 문제를 해결하기 위해
모델 하나당 별도 프로세스로 실행하도록 변경.

사용법:
    python export_tensorrt_jetson_single.py yolov10s.pt

여러 모델을 셸에서 순회 (각 호출이 독립 프로세스라 메모리 완전히 정리됨):
    for m in yolov8n yolov8s yolov8m yolo11n yolo11s yolo11m \
             yolov10n yolov10s yolov10m; do
