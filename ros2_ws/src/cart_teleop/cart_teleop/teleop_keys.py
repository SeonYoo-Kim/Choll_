...
# ── 기본값 ─────────────────────────────────────────────────────────────────
# 2026-08-04 실기에서 확인한 범위를 넘지 않는 값.
#   linear 0.13 m/s  = 바퀴 2.0 rad/s (speed_profile slow 상한과 동일)
#   angular 0.60 rad/s = 바퀴 ±1.754 rad/s (L=0.38 실측 기준, slow 상한 이내)
# 최종 상한 방어선은 이 모듈이 아니라 Bridge의 `speed_profile:=slow`다.
DEFAULT_MAX_LINEAR_MPS = 0.13
DEFAULT_MAX_ANGULAR_RPS = 0.60
...
class TeleopState:
    ...
    def __init__(
        ...
        *,
        ...
        # 기본값은 최대 단계 — 요구된 기본 속도(0.13 / 0.60)가 곧 상한값이다.
